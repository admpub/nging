var App = (function () {
  ...
  function pageAside() {
    var pageKey = window.location.pathname
      .replace(/^\//, "")
      .replace(/[^\w]/g, "-"); // + '-' + window.location.search.replace(/[^\w]/g, '_');
    $(".page-aside").each(function (index) {
      var pKey = $(this).attr("page-key") || pageKey;
      var aside = $(this),
        key = pKey + ".page-aside-" + index;
      $(this)
        .find(".header > .collapse-button")
        .on("click", function () {
  ...
  function showMenu(_this, e) {
    if (
      ($("#cl-wrapper").hasClass("sb-collapsed") ||
        ($(window).width() >= 753 && $(window).width() <= 964)) &&
      $("ul", _this).length > 0
    ) {
      ...
      if (!$(".dropdown-header", _this).length) {
        var head =
          '<li class="dropdown-header">' + $(_this).children().html() + "</li>";
        menu.prepend(head);
      }
  ...
  function passwordInputShowPassword(container) {
    var e,
      s =
        'a.show-password[data-target]:not([pwdshow]),a[data-toggle="password"]:not([pwdshow])';
    if (!container) {
      e = $(s);
    } else {
      e = $(container).find(s);
    ...
    e.on("click", function () {
      var target = $(this).data("target")
        ? $($(this).data("target"))
        : $(this).parent(".input-group-addon,.input-group-btn").prev("input");
  ...
  return {
    ...
    init: function (options) {
      ...
      function onWinResize(wrapper) {
        var $pcont = $("#pcont");
        $pcont
          .children(".cont-container")
          .width($(window).width() - $pcont.offset().left - 1);
      }
      ...
      if (config.bootstrapSwitch) {
        if ($(".switch:not(.has-switch)").length > 0)
    ...
    autoFixedThead: function (prefix) {
      ...
      var top = function () {
        return $(window).width() >= 753 ? $("#head-nav").height() : 0;
      };
      ...
      switch (position) {
        case "left":
          ...
          if (li.parent("ul.sub-menu").length > 0) {
            ...
            if (op2.length > 0)
              op2.removeClass("open").find("li.active").removeClass("active");
          }
          $("#leftnav > .open")
            .removeClass("open")
        ...
        case "top":
          var topnavDropdown = li.closest("ul.dropdown-menu.from-top");
          if (topnavDropdown.length > 0) {
            if (topnavDropdown.hasClass("col-menu-2")) {
              topnavDropdown.find("li.active").removeClass("active");
            }
            siblings = topnavDropdown
              .parent("li")
              .addClass("active")
              .siblings("li.active");
          }
          if (siblings.length > 0) {
            siblings
              .removeClass("active")
              .children("ul.dropdown-menu.from-top")
              .find("li.active")
              .removeClass("active");
          }
          break;
    ...
    message: function (options, sticky) {
      ...
      if (typeof options != "object")
        options = { text: options, class_name: "clean" };
      if (typeof options.type != "undefined" && options.type)
        options.class_name = options.type;
      ...
      switch (options.class_name) {
        ...
        case "info":
          if (options.title)
            options.title =
              '<i class="fa fa-info-circle"></i> ' + App.t(options.title);
          break;
        ...
        case "danger":
          if (options.title)
            options.title =
              '<i class="fa fa-comment-o"></i> ' + App.t(options.title);
          break;

        case "warning":
          if (options.title)
            options.title =
              '<i class="fa fa-warning"></i> ' + App.t(options.title);
          break;

        case "success":
          if (options.title)
            options.title =
              '<i class="fa fa-check"></i> ' + App.t(options.title);
          break;
      }
    ...
    attachAjaxURL: function (elem) {
      ...
      $(elem).on("click", "[data-ajax-url]", function () {
        var a = $(this),
          confirmMsg = a.data("ajax-confirm");
        ...
        var url = a.data("ajax-url"),
          method = a.data("ajax-method") || "get",
          params = a.data("ajax-params") || {},
          title = a.attr("title") || App.i18n.SYS_INFO,
          accept = a.data("ajax-accept") || "html",
          target = a.data("ajax-target"),
          callback = a.data("ajax-callback"),
          toggle = a.data("ajax-toggle"),
          onsuccess = a.data("ajax-onsuccess"),
          reload = a.data("ajax-reload") || false;
    ...
    startLazyload: function (elem) {
      if (elem == null) elem = document;
      $(elem)
        .find("[lazyload-url]:not([loaded])")
        .each(function () {
          var a = $(this);
          a.attr("loaded", "1");
          var url = a.attr("lazyload-url"),
            method = a.attr("lazyload-method") || "get",
            params = a.attr("lazyload-params") || {},
            title = a.attr("title") || App.i18n.SYS_INFO,
            accept = a.attr("lazyload-accept") || "html",
            target = a.attr("lazyload-target"),
            onsuccess = a.attr("lazyload-onsuccess");
          if (!title) title = a.text();
          ...
          $.ajax({
            ...
            success: function (r) {
              ...
              if (accept == "json") {
                if (r.Code != 1) {
                  $(target).html(
                    '<span class="lazyload-error text-danger">' +
                      r.Info +
                      "</span>&nbsp;",
                  );
                  return;
    ...
    attachPjax: function (elem, callbacks, timeout) {
      ...
      $(document)
        .on("click", elem + "[data-pjax]", function (event) {
          var container = $(this).data("pjax"),
            keepjs = $(this).data("keepjs");
          ...
          if (toggleClass) {
            ...
            if (parent) {
              ...
              switch (parent) {
                case "parent":
                  target = $(this).parent(parentElem);
                  break;
                case "parents":
                  target = $(this).parents(parentElem);
                  break;
                case "closest":
                  target = $(this).closest(parentElem);
                  break;
                default:
                  target = $(this).parent(parentElem);
                  break;
        ...
        .on("pjax:complete", function (evt, xhr, textStatus, option) {
          ...
          if (options.oncomplete)
            options.oncomplete(evt, xhr, textStatus, option);
        })
        ...
        .on("pjax:end", function (evt, xhr, option) {
          ...
          if (option.container) {
            ...
            $(option.container + " .table-responsive[data-pattern]").each(
              function () {
          ...
          if (option.type == "GET")
            $("#global-search-form").attr("action", option.url);
          App.formWithNotify();
        });
    },
    attachTurn: function (elem, options) {
      ...
      switch ($(o.elem)[0].tagName.toUpperCase()) {
        case "INPUT": //radio/checkbox
          $(o.elem + ":checked").trigger(o.event);
          break;
        case "SELECT":
    ...
    notifyRecvDefault: function (message, percent, disableAutoClose) {
      var containerId = "notify-default-shower",
        bodyId = "notify-default-shower-body";
      var elem = "#" + containerId,
        bodyE = "#" + bodyId;
      if (disableAutoClose == null) disableAutoClose = false;
      if (disableAutoClose && !message) {
        if ($(elem).length > 0)
          $(bodyE).slideUp("slow", function () {
            $(elem).remove();
          });
        return;
      }
      if ($(elem).length < 1) {
        $("body").append(
          '<div id="' +
            containerId +
            '" style="min-height:30px;position:fixed;display:block;top:50px;text-align:center;z-index:2200;width:100%;pointer-events:none">\
				<div id="' +
            bodyId +
            '" style="padding:0 10px 1px 10px;background:rgba(255,166,0,0.8);border:1px solid salmon;border-radius:5px;display:inline-block;margin:0 auto;color:black;text-shadow:0 1px 1px #ccc;font-size:1em;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;max-width:100%;line-height:30px;pointer-events:auto"></div>\
				</div>\
				</div>',
        );
        if (!disableAutoClose) {
    ...
    formWithNotify: function () {
      ...
      if ($input.length < 1) {
        $form.append(
          '<input type="hidden" name="notifyClientID" value="' +
            clientID +
            '">',
        );
    ...
    messageNotify: function (title, d) {
      ...
      if (d.sound) {
        audioHTML =
          '<audio style="visibility:hidden;position:absolute;bottom:0;" controls="controls" hidden="true" src="' +
          d.sound +
          '"></audio>';
      }
      App.lastMessageId = App.message({
        ...
        text:
          ...
          "</strong>" +
          (d.isAdmin
            ? '<span class="badge badge-warning">' + App.t("管理员") + "</span>"
            : "") +
          ':<br /><a href="' +
          d.url +
          '" class="text-white">' +
    ...
    notifyListen: function () {
      var messageCount = { notify: 0, element: 0, modal: 0 },
        ...
        messageHandle = function (message) {
          ...
          if (!m) {
            App.message({
              text: message || "Websocket Server is disconnected",
              type: "error",
            });
            return false;
          }
          ...
          switch (m.mode) {
            ...
            case "element":
              ...
              if (c.length < 1) {
                ...
                App.notifyRecvDefault(
                  m.content,
                  m.progress ? m.progress.percent : 0,
                );
                return false;
              }
            ...
            case "modal":
              ...
              if (!c.data("shown")) {
                ...
                c.data("shown", true);
                var mbody = c.find(".modal-body"),
                  mbodyUL = mbody.children("ul.modal-body-ul");
                if (mbodyUL.length < 1) {
                  mbody.html(
                    '<ul class="modal-body-ul" id="notify-modal-' +
                      m.type +
                      '-container"><li>' +
                      message +
                      "</li></ul>",
            ...
            case "notify":
              ...
              if (c.length < 1) {
                App.message({
                  title: App.i18n.SYS_INFO,
                  text: '<ul id="notice-message-container" class="no-list-style" style="max-height:500px;overflow-y:auto;overflow-x:hidden"></ul>',
                  sticky: true,
                });
        ...
        connect = function (onopen) {
          if (!!window.EventSource) {
            ...
            var source = new EventSource(BACKEND_URL + "/user/sse", {
              headers: headers,
            });
    ...
    checkedAll: function (ctrl, target) {
      return $(target)
        .not(":disabled")
        .prop("checked", $(ctrl).prop("checked"));
    },
        ...
        case "info":
          ...
          return (
            '<div class="alert alert-info">\
								...
								<i class="fa fa-info-circle sign"></i><strong>' +
            title +
            "</strong> " +
            content +
            "</div>"
        ...
        case "warn":
          ...
          return (
            '<div class="alert alert-warning">\
								<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>\
								<i class="fa fa-warning sign"></i><strong>' +
            title +
            "</strong> " +
            content +
            "</div>"
        ...
        case "error":
          ...
          return (
            '<div class="alert alert-danger">\
								<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>\
								<i class="fa fa-times-circle sign"></i><strong>' +
            title +
            "</strong> " +
            content +
            "</div>"
        ...
        default:
          ...
          return (
            '<div class="alert alert-success">\
								<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>\
								<i class="fa fa-check sign"></i><strong>' +
            title +
            "</strong> " +
            content +
            "</div>"
    ...
    showDbLog: function (results, container) {
      ...
      for (var j = 0; j < results.length; j++) {
        ...
        if (result.SQLs && result.SQLs.length > 0) {
          for (var i = 0; i < result.SQLs.length; i++)
            s += '<code class="wrap">' + result.SQLs[i] + "</code>";
        } else {
    ...
    insertAtCursor: function (myField, myValue, posStart, posEnd) {
      ...
      if (document.selection) {
        ...
        sel.text = myValue;
        sel.select();
      } /* MOZILLA/NETSCAPE support */ else if (
              ...
              if ($(elem).next(".fixed-placeholder").length > 0) {
                ...
                $(elem).after(
                  '<div style="width:' +
                    ...
                    "px;height:" +
                    h +
                    'px" class="fixed-placeholder"></div>',
                );
              }
            }
          }); //on-scroll
    ...
    topFloatRawThead: function (elems, top) {
      ...
      $(elems)
        ...
        .each(function () {
          $(this).attr("disabled-fixed", "fixed");
          var elem = this,
            table = $(elem).parent("table"),
            reponsive = table.closest(".table-responsive");
          var scrollable = reponsive.length > 0;
          var offsetY = $(elem).offset().top,
            marginTop = Number(
              String(table.css("margin-top")).replace(/px$/, ""),
            );
          ...
          var setSize = function (init) {
            ...
            var width = $(elem).width(),
              ratio = 1;
            if (!init) {
              if (Math.abs(tableWidth - width) > 1) {
                //避免抖动
            ...
            var oldRatio = $(elem).data("ratio") || 0;
            $(elem).data("ratio", ratio);
            table.data("offset-left", table.offset().left); //记录左侧偏移
            table.data("scroll-left", winScrollLeft); //记录左侧滚动条
            var cols =
                table.children("colgroup").length > 0
                  ? table.children("colgroup").children("col")
                  : table.children("col"),
              tds = $(elem).find("td,th");
            if (cols.length < 1) {
              var html = "";
            ...
            var tbodyTds =
              table.children("tbody").length > 0
                ? table.children("tbody").children("tr:first").find("td")
                : table.children("tr:first").find("td");
            tds.each(function (index) {
              var col = cols.eq(index),
                td = tbodyTds.eq(index);
              var w =
                !td || col.outerWidth() > td.outerWidth()
                  ? col.outerWidth()
                  : td.outerWidth();
              $(this).css({ width: w });
            });
          ...
          $(window).on("scroll resize", function () {
            setSize();
            var scrollH = $(this).scrollTop(),
              maxOffsetY = table.height() + offsetY - $(elem).outerHeight() * 2;
            ...
            if (table.height() > $(window).height()) {
              if (!$(elem).hasClass("always-top")) {
                ...
                table.css({
                  "margin-top":
                    table.data("originalMarginTop") + $(elem).height(),
                });
              }
                ...
                if (!reponsive.data("scroll-thead")) {
                  reponsive
                    .on("scroll", function () {
                      $(elem).css({
                        left:
                          reponsive.offset().left + $(this).scrollLeft() * -1,
                      });
                    })
    ...
    topFloatThead: function (elems, top, clone) {
      ...
      $(elems)
        ...
        .each(function () {
          ...
          var eBox = $(
            '<div class="always-top" style="overflow:hidden;display:block;width:' +
              (scrollable ? reponsive.width() : table.width()) +
              "px;top:" +
              topF() +
              'px;box-shadow:0 1px 5px rgba(0,0,0,0.3)"></div>',
          );
          var eCopy = $(
            '<table class="' +
              table.attr("class") +
              '" style="background-color:white;table-layout:fixed;position:relative" id="tableCopy' +
              cid +
              '"></table>',
          );
          ...
          var setSize = function (init) {
            ...
            var cols = hCopy.find("td,th"),
              rawCols = $(elem).find("td,th");
    ...
    getImgNaturalDimensions: function (oImg, callback) {
      ...
      if (oImg) nImg.src = oImg.src;
    },
    reportBug: function (url) {
      $.post(
        url,
        { panic: $("#panic-content").html(), url: window.location.href },
        function (r) {},
        "json",
      );
    },
    ...
    format: function (raw, xy, formatters) {
      if (formatters) {
        if (typeof formatters[xy.y] == "function")
          return formatters[xy.y](raw, xy);
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        for (var k in row) {
          ...
          bd +=
            ...
            k +
            '">' +
            App.format(row[k], { x: i, y: k }, formatters) +
            "</td>";
        }
        bd += "</tr>";
      }
    ...
    logShow: function (elem, trigger, pipe, tableGenerator) {
      ...
      if (pipe == null) pipe = "";
      var tableGen =
        tableGenerator && typeof tableGenerator == "function"
          ? tableGenerator
          : function (list) {
      ...
      var done = function (a) {
        ...
        var contentID = "log-show-content",
          contentE = "#" + contentID;
            ...
            $.get(
              ...
              function (r) {
                if (r.Code == 1) {
                  ...
                  if (
                    typeof r.Data.charset != "undefined" &&
                    $("#log-show-charset").length > 0 &&
                    $("#log-show-charset").val() != r.Data.charset
                  ) {
                    $("#log-show-charset")
                      .find('option[value="' + r.Data.charset + '"]')
                      .prop("selected", true);
                  }
                  $(window).trigger("resize");
                } else {
    ...
    tableSorting: function (table) {
      ...
      $(table + "[sort-current] [sort]")
        ...
        .on("click", function (e) {
          ...
          var trigger =
            thead.attr("sort-trigger") || thead.data("sort-trigger");
          var sort = $(this).attr("sort");
          if (current && (current == sort || current == "-" + sort)) {
            var reg = /^\-/;
            current = reg.test(current)
              ? current.replace(reg, "")
              : "-" + current;
          } else {
            current = sort;
            ...
            if (setto) {
              ...
              $.get(
                ...
                function (r, status, xhr) {
                  if (
                    String(xhr.getResponseHeader("Content-Type")).split(
                      ";",
                    )[0] == "application/json"
                  ) {
                    try {
                  ...
                  if (
                    $(setto).length > 0 &&
                    $(setto)[0].tagName.toUpperCase() == "TBODY"
                  ) {
                    var thead = $this.parents("[sort-current]");
                    var current = thead.attr("sort-current");
                    var isDesc = current.substring(0, 1) == "-";
    ...
    switchStatus: function (a, type, editURL, callback, dataBuilder) {
      ...
      var that = $(a),
        status = a.checked ? checkedValue : uncheckedValue,
        data = { id: that.data("id") };
      data[type] = status;
      if (dataBuilder && typeof dataBuilder == "function")
        data = dataBuilder(data, type, status);
      if (String(editURL).charAt(0) != "/")
        editURL = BACKEND_URL + "/" + editURL;
      $.get(
        editURL,
        data,
        function (r) {
          ...
          App.message({
            ...
            time: 5000,
            sticky: false,
            class_name: r.Code == 1 ? "success" : "error",
          });
    ...
    removeSelected: function (elem, postField, removeURL, callback) {
      return App.opSelected(
        elem,
        postField,
        removeURL,
        callback,
        App.i18n.CONFIRM_REMOVE,
        App.i18n.PLEASE_SELECT_FOR_REMOVE,
      );
    },
    removeSelected2: function (obj, elem, postField, removeURL, callback) {
      return App.opSelected2(
        obj,
        elem,
        postField,
        removeURL,
    ...
    opSelected2: function (
      ...
      if (data.length < 1) {
        App.message({
          title: App.i18n.SYS_INFO,
          text: unselectedMsg || App.i18n.PLEASE_SELECT_FOR_OPERATE,
          type: "warning",
        });
        return false;
      }
      ...
      if (_this && _this !== null && typeof _this == "object") {
        if (String($(_this).data("method")).toLowerCase() == "post")
          method = "post";
      }
      $[method](
        removeURL,
    ...
    float: function (elem, mode, attr, position, options) {
      ...
      if (!position)
        position = "5-7"; //两个数字分别代表trigger(触发对象)-target(浮动层)，（各个数字的编号从矩形框的左上角开始，沿着顺时针开始旋转来进行编号，然后再从上中部开始沿着顺时针开始编号进行。也就是1、2、3、4分别代表左上角、右上角、右下角、左下角；5、6、7、8分别代表上中、右中、下中、左中）
      else {
        ...
        if (arr.length == 1) {
          switch (arr[0]) {
            case "bottom":
              val.push(App.floatAtoi("top"));
              break;
            case "right":
              val.push(App.floatAtoi("left"));
              break;
            case "top":
              val.push(App.floatAtoi("bottom"));
              break;
            case "left":
              val.push(App.floatAtoi("right"));
              break;
            case "leftBottom":
              val.push(App.floatAtoi("rightBottom"));
              break;
            case "rightBottom":
              val.push(App.floatAtoi("leftBottom"));
              break;
            case "leftTop":
              val.push(App.floatAtoi("rightTop"));
              break;
            case "rightTop":
              val.push(App.floatAtoi("leftTop"));
              break;
            default:
              val.push(p);
              break;
    ...
    plotTooltip: function (x, y, contents) {
      $("<div id='plot-tooltip'>" + contents + "</div>")
        .css({
          position: "absolute",
          display: "none",
          top: y + 5,
    ...
    plotHover: function (elem, formatter) {
      $(elem).on("plothover", function (event, pos, item) {
        ...
        $("#plot-tooltip").remove();
        if (formatter == null)
          formatter = function (event, pos, item) {
        ...
        App.plotTooltip(
          item.pageX,
          item.pageY,
          formatter.call(this, event, pos, item),
        );
      });
    },
    template: function (elem, jsonData, onSwitchPage) {
      var tplId = $(elem).attr("tpl"),
        arr = String(tplId).split("=>");
      tplId = $.trim(arr[0]).replace(/^#/, "");
      if ($("#" + tplId).length < 1)
        throw new Error(
          "not found template: " +
            (typeof elem == "string" ? elem : $(elem).attr("id")) +
            "=>" +
            tplId,
        );
      if (arr.length > 1) {
        elem = $.trim(arr[1]);
        if ($(elem).length < 1)
          throw new Error("[template] not found wrapper: " + elem);
      }
      $(elem).html(template(tplId, jsonData));
      if (onSwitchPage == null) return;
      $(elem)
        .find("ul.pagination li > a[page]")
        .on("click", function () {
    ...
    captchaUpdate: function ($form, resp) {
      if (
        App.captchaHasError(resp.Code) &&
        resp.Data &&
        typeof resp.Data.captchaIdent !== "undefined"
      ) {
        ...
        if (resp.Data.captchaName)
          $form.find('input[name="' + resp.Data.captchaName + '"]').focus();
      }
    },
    ...
    treeToggle: function (elem, options) {
      ...
      $(elem).on("click", ".tree-toggler", function () {
        ...
        if (typeof options.onclick === "function")
          options.onclick.apply(that, arguments);
      ...
      if (options.expandFirst)
        $(elem + " label.tree-toggler:first").trigger("click");
    },
    ...
    makeCheckerForUpgrade: function (
      max,
      version,
      successCallback,
      errorCallback,
    ...
    showUpgradeInfo: function (version) {
      ...
      if (upgradeBtn.length > 0) {
        ...
        upgradeBtn.on("click", function () {
          ...
          var upgradeModal = $("#self-upgrade-modal"),
            version = upgradeModal.data("version");
          $.post(
            BACKEND_URL + "/manager/upgrade",
            {
              download: true,
              notifyClientID: App.clientID["notify"] || "any",
      ...
      $.get(
        ...
        function (r) {
          ...
          upgradeModal.niftyModal("show", {
            /*closeOnClickOverlay:false,afterClose:function(modal){}*/
          });
        },
        "json",
      );
