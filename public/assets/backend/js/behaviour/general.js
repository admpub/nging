	...
	return {
		...
		markNav: function (curNavA, position) {
			...
			switch (position) {
				...
				case 'top':
					li.closest('.from-top').parent('li').addClass("active").siblings('li.active').removeClass('active');
					break;
			}
		},
		unmarkNav: function (curNavA, position) {
			...
			switch (position) {
				...
				case 'top':
					...
					if (siblings.length > 0) {
						siblings.removeClass('active').children('ul.dropdown-menu.from-top').find('li.active').removeClass('active');
					}
					break;
			}
		},
		...
		topFloatRawThead: function (elems, top) {
			...
			$(elems).not('[disabled-fixed]').each(function () {
				...
				var setSize = function (init) {
					...
					if (!init) {
						if (Math.abs(tableWidth - width) > 1) {//避免抖动
					...
					var cols = table.children('colgroup').length>0?table.children('colgroup').children('col'):table.children('col'), 
					tds = $(elem).find('td,th');
					if (cols.length < 1) {
						var html = '';
		...
		getImgNaturalDimensions: function (oImg, callback) {
			if (oImg && oImg.naturalWidth) { // 现代浏览器
