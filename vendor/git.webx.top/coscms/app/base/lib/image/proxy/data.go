...
func (o Options) String() string {
	...
	if o.Rotate != 0 {
		fmt.Fprintf(buf, ",%s%d", optRotatePrefix, o.Rotate)
	}
	if o.FlipVertical {
	...
	if o.Quality != 0 {
		fmt.Fprintf(buf, ",%s%d", optQualityPrefix, o.Quality)
	}
	if o.Signature != "" {
		fmt.Fprintf(buf, ",%s%s", optSignaturePrefix, o.Signature)
	}
	return buf.String()
}
...
// 	http://localhost/http://example.com/image.jpg
func NewRequest(r *http.Request, baseURL *url.URL,
	resVKey string, validToken bool, crossParams bool) (*Request, error) {
	var err error
	req := &Request{Original: r}
	var path string
	if resVKey != "" && r.URL.RawQuery != "" {
		if resVKey == "-" {
			path = r.URL.RawQuery
			r.URL.RawQuery = ""
		} else {
	...
	if err != nil || !req.URL.IsAbs() {
		...
		parts := strings.SplitN(path, "/", paramNum)
		if len(parts) != paramNum {
		...
		req.Setting = opts + "/" + resURL
		if crossParams && r.URL.RawQuery != "" {
			req.Setting += "?" + r.URL.RawQuery
		}
		var err error
		req.URL, err = url.Parse(resURL)
		if err != nil {
			return nil, URLError{fmt.Sprintf("unable to parse remote URL: %v", err), r.URL}
		}

		req.Options = ParseOptions(opts)
	}
