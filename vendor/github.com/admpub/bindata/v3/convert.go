...
func Translate(c *Config) error {
	...
	for _, input := range c.Input {
		err = findFiles(input.Path, prefixes, input.Recursive, &toc, c.Ignore, knownFuncs, visitedPaths, c.Minify)
		if err != nil {
			return err
		}
	}
...
func findFiles(dir string, prefixes []string, recursive bool, toc *[]Asset, ignore []*regexp.Regexp, knownFuncs map[string]int, visitedPaths map[string]bool, minify []*regexp.Regexp) error {
	dirpath := dir
	if len(prefixes) > 0 {
	...
	for _, file := range list {
		...
		if file.IsDir() {
			if recursive {
				...
				findFiles(recursivePath, prefixes, recursive, toc, ignore, knownFuncs, visitedPaths, minify)
			}
			continue
			...
			if _, ok := visitedPaths[linkPath]; !ok {
				...
				findFiles(asset.Path, prefixes, recursive, toc, ignore, knownFuncs, visitedPaths, minify)
			}
			continue
		}
