...
func easyjsonC5a4559bDecodeGithubComChromedpCdprotoAudits22(in *jlexer.Lexer, out *BounceTrackingIssueDetails) {
	isTopLevel := in.IsStart()
	if in.IsNull() {
		if isTopLevel {
			in.Consumed()
		...
		case "trackingSites":
			if in.IsNull() {
				...
				for !in.IsDelim(']') {
					var v7 string
					v7 = string(in.String())
					out.TrackingSites = append(out.TrackingSites, v7)
					in.WantComma()
				}
				in.Delim(']')
			}
...
func easyjsonC5a4559bEncodeGithubComChromedpCdprotoAudits22(out *jwriter.Writer, in BounceTrackingIssueDetails) {
	out.RawByte('{')
	first := true
	_ = first
	{
		...
		if in.TrackingSites == nil && (out.Flags&jwriter.NilSliceAsEmpty) == 0 {
			...
			out.RawByte('[')
			for v8, v9 := range in.TrackingSites {
				if v8 > 0 {
					out.RawByte(',')
				}
